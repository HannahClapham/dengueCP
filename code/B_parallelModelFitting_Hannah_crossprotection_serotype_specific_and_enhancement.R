...
#HClambdasbw <- seq( lambda.max*26,0, length.out=grid.size)
...
tmp <- get.cross.protect.data(bkk.dengue.all.cases,
                              ...
                              delta.cpss=zeros, cr.protect.lag.cpss=zeros, lambda=NULL,
...
for ( p in 1:nrow(paramsexp)){
  ...
  lambdacpss[1:4,1:4]<-paramsexp[p,"lambdas1.vec"] 
  #HC lambdacpss[2,1:4]<-paramsexp[p,"lambdas2.vec"] 
  #HC lambdacpss[3,1:4]<-paramsexp[p,"lambdas3.vec"]  
  #HC lambdacpss[4,1:4]<-paramsexp[p,"lambdas4.vec"] 
  
  deltacpss[1:4,1:4]<-paramsexp[p,"deltas.vec"] 
  
  #HC deltacpss[2,1:4]<-paramsexp[p,"deltas2.vec"] 
  #HC deltacpss[3,1:4]<-paramsexp[p,"deltas3.vec"]  
  #HC deltacpss[4,1:4]<-paramsexp[p,"deltas4.vec"] 
  ...
  tmp <- get.cross.protect.data(bkk.dengue.all.cases,
                                ...
                                delta.cpss=deltacpss, cr.protect.lag.cpss=kcpss,
